...
import {
  Terminal, Play, Square, Zap, AlertCircle, RotateCcw,
  Wifi, WifiOff, Link as LinkIcon,
} from 'lucide-react'
...
import { logGenApi, jobsApi, type LogEntry } from '@/lib/api'
...
const LEVEL_BADGE: Record<string, 'default' | 'secondary' | 'warning' | 'destructive'> = {
  info: 'default',
  debug: 'secondary',
  warn: 'warning',
  error: 'destructive',
...
const ERROR_TYPES = ['database', 'authentication', 'network', 'file', 'memory', 'payment']
...
export function LogMonitor() {
  ...
  const doPoll = useCallback(async () => {
    try {
      const { logs: entries, latest } = await logGenApi.recentLogs(latestSeq || undefined)
      if (entries.length) {
        appendLogs(entries)
        setLatestSeq(latest)
      }
  ...
  useEffect(() => { doPollRef.current = doPoll }, [doPoll])
  ...
  const startNormal = useMutation({
    ...
    onSuccess: () => { toast({ title: 'Normal log generation started' }); refetchStatus() },
    onError: () => toast({ title: 'Log generator unreachable', variant: 'destructive' }),
  })
  const stopNormal = useMutation({
    mutationFn: logGenApi.stopNormal,
    onSuccess: () => { toast({ title: 'Normal log generation stopped' }); refetchStatus() },
    onError: () => toast({ title: 'Log generator unreachable', variant: 'destructive' }),
  })
  const startErrors = useMutation({
    mutationFn: logGenApi.startErrors,
    onSuccess: () => { toast({ title: 'Error generation started' }); refetchStatus() },
    onError: () => toast({ title: 'Log generator unreachable', variant: 'destructive' }),
  })
  const stopErrors = useMutation({
    mutationFn: logGenApi.stopErrors,
    onSuccess: () => { toast({ title: 'Error generation stopped' }); refetchStatus() },
    onError: () => toast({ title: 'Log generator unreachable', variant: 'destructive' }),
  })
  const triggerBatch = useMutation({
    ...
    onSuccess: (data: { triggered: number }) => {
      ...
      if (linkedJobId) setTimeout(() => qc.invalidateQueries({ queryKey: ['anomalies', linkedJobId] }), 3000)
    },
    onError: () => toast({ title: 'Failed to trigger batch', variant: 'destructive' }),
  })
  const triggerSingle = useMutation({
    mutationFn: () => logGenApi.generateError(errorType ? { type: errorType } : undefined),
    onSuccess: () => {
      toast({ title: 'Error log triggered' })
      if (linkedJobId) setTimeout(() => qc.invalidateQueries({ queryKey: ['anomalies', linkedJobId] }), 3000)
    },
    onError: () => toast({ title: 'Failed to trigger error', variant: 'destructive' }),
  })
    ...
    <div className="flex h-full flex-col p-6 gap-4 overflow-auto">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">Real-time logs from the dummy generator</p>
        </div>
        <div className="flex items-center gap-2">
          {genOnline
            ? <Badge variant="success" className="gap-1.5"><Wifi className="h-3 w-3" />Generator Online</Badge>
            : <Badge variant="secondary" className="gap-1.5"><WifiOff className="h-3 w-3" />Generator Offline</Badge>
          }
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <div className="space-y-4">
          <Card>
            <CardHeader><CardTitle className="flex items-center gap-2 text-sm"><Zap className="h-4 w-4 text-yellow-400" />Generator Controls</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <p className="text-xs font-medium text-gray-400">Normal Logs</p>
                <div className="flex gap-2">
                  <Button size="sm" variant="secondary" className="flex-1"
                    disabled={!genOnline || !!genStatus?.generation?.normalLogs || startNormal.isPending}
                    onClick={() => startNormal.mutate()}>
                    <Play className="h-3.5 w-3.5" /> Start
                  </Button>
                  <Button size="sm" variant="outline" className="flex-1"
                    disabled={!genOnline || !genStatus?.generation?.normalLogs || stopNormal.isPending}
                    onClick={() => stopNormal.mutate()}>
                    <Square className="h-3.5 w-3.5" /> Stop
                  </Button>
                </div>
                <p className="text-xs text-gray-600">
                  {genStatus ? (genStatus.generation.normalLogs
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Button size="sm" variant="destructive" className="flex-1"
                    disabled={!genOnline || !!genStatus?.generation?.errors || startErrors.isPending}
                    onClick={() => startErrors.mutate()}>
                    <Play className="h-3.5 w-3.5" /> Start
                  </Button>
                  <Button size="sm" variant="outline" className="flex-1"
                    disabled={!genOnline || !genStatus?.generation?.errors || stopErrors.isPending}
                    onClick={() => stopErrors.mutate()}>
                    <Square className="h-3.5 w-3.5" /> Stop
                  </Button>
                </div>
                <p className="text-xs text-gray-600">
                  {genStatus ? (genStatus.generation.errors
              ...
              <div className="space-y-2">
                <p className="text-xs font-medium text-gray-400">Manual Trigger</p>
                <Select value={errorType} onValueChange={setErrorType}>
                  <SelectTrigger className="h-8"><SelectValue placeholder="Random error type" /></SelectTrigger>
                  <SelectContent>
              ...
              <div className="space-y-2">
                <p className="text-xs font-medium text-gray-400">Batch Trigger</p>
                <div className="flex gap-2">
                  <Input type="number" value={batchCount} onChange={e => setBatchCount(e.target.value)}
                    min={1} max={50} className="h-8 w-20" />
                  <Button size="sm" variant="outline" className="flex-1"
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-sm">
                <LinkIcon className="h-4 w-4 text-blue-400" />Linked Job Anomalies
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              <Select value={linkedJobId} onValueChange={setLinkedJobId}>
                <SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Select job to watch..." /></SelectTrigger>
                <SelectContent>
                  {(jobs.data || []).map(j => <SelectItem key={j.id} value={j.id}>{j.name}</SelectItem>)}
                </SelectContent>
              </Select>
              ...
              {linkedJobId && (
                <div className="space-y-1">
                  {anomaliesQuery.isLoading ? (
                    ...
                    <p className="text-xs text-gray-600">No anomalies yet — trigger some errors above (Fluent Bit must be routing to this job).</p>
                  ) : (
                    <>
                      <p className="text-xs text-gray-500">{anomaliesQuery.data?.total} total anomalies</p>
                      <div className="space-y-1.5 max-h-48 overflow-y-auto">
                        {(anomaliesQuery.data?.data || []).map(a => (
                          <div key={a.id} className="rounded-md bg-white/3 px-3 py-2 space-y-0.5">
                            <div className="flex items-center gap-2">
                              <Badge variant={SEV_VARIANT[a.severity]}>{a.severity}</Badge>
                              <Badge variant={a.status === 'open' ? 'default' : a.status === 'in_progress' ? 'warning' : 'success'}>
                                {a.status.replace('_', ' ')}
                              </Badge>
                            </div>
        ...
        <div className="lg:col-span-2 flex flex-col gap-3">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {polling && <span className="inline-block h-2 w-2 rounded-full bg-green-400 animate-pulse" />}

            <Select value={filterLevel} onValueChange={setFilterLevel}>
              <SelectTrigger className="h-8 w-28"><SelectValue /></SelectTrigger>
              <SelectContent>
                {['all', 'info', 'debug', 'warn', 'error', 'critical'].map(l => (
                  <SelectItem key={l} value={l}>{l}</SelectItem>
                ))}
              </SelectContent>
            </Select>

            <Button size="sm" variant="ghost" onClick={() => { setLogs([]); setLatestSeq(0) }}>
              <RotateCcw className="h-3.5 w-3.5" /> Clear
            </Button>
            <span className="text-xs text-gray-600 ml-auto">{filteredLogs.length} lines</span>
            ...
            <div className="h-112 overflow-y-auto p-3 font-mono text-xs space-y-0.5">
              {filteredLogs.length === 0 ? (
                ...
                filteredLogs.map(log => (
                  <div key={log.seq} className="flex items-start gap-2 hover:bg-white/2 px-1 py-0.5 rounded group">
                    <span className="text-gray-700 shrink-0 tabular-nums">
                      {new Date(log.timestamp).toLocaleTimeString()}
                    </span>
                    <Badge variant={LEVEL_BADGE[log.level] ?? 'secondary'} className="shrink-0 text-[10px] px-1.5">
                      {log.level?.toUpperCase()}
                    </Badge>
                    {log.service && (
                      <span className="text-purple-400 shrink-0">[{String(log.service)}]</span>
                    )}
                    <span className={LEVEL_COLORS[log.level] || 'text-gray-300'}>{log.message}</span>
                  </div>
                ))
              )}
              <div ref={logsEndRef} />
